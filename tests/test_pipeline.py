...
class DummyAgent(BaseAgent):
    ...
    def __init__(self, name: str = "dummy", should_fail: bool = False):
        super().__init__(name=name, description="Dummy agent for testing")
    ...
    def run(self, input_data: str, **kwargs) -> AgentResult:
        ...
        return AgentResult(
            success=True,
            content=f"Processed by {self.name}: {input_data}",
            agent_name=self.name,
        )
...
def test_pipeline_empty_steps(pipeline):
    ...
    assert "No steps provided" in result.content or "empty" in result.content.lower()
...
def test_pipeline_missing_agent(pipeline):
    ...
    result = pipeline.run_chain(steps)
    assert result.success is False
    assert "not registered" in result.content.lower() or "not found" in result.content.lower()
...
def test_pipeline_step_failure(pipeline):
    ...
    assert "Error in failing_agent" in result.content
...
def test_cli_chain_command_empty_pipeline():
    ...
    assert res.exit_code != 0
    assert "Error" in res.output or "No agents" in res.output
