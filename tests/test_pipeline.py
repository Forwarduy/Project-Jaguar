"""Tests for pipeline orchestration and CLI REPL/chain commands."""
...
def test_pipeline_agent_kwargs_execution():
    ...
    pipeline = AgentPipeline(mock_registry)
    result = pipeline.run_chain([
        {"agent": "outreach", "arg": "Target", "kwargs": {"message_context": "Priority"}}
    ])

    assert result.success is True
    mock_agent.run.assert_called_once_with("Target", message_context="Priority")
...
def test_cli_chain_command_success(mock_registry):
    mock_agent = MagicMock()
    mock_agent.run.return_value = AgentResult(success=True, content="Chain result")
    mock_registry.get.return_value = lambda: mock_agent

    result = runner.invoke(app, ["chain", "research,plan", "Quantum Computing"])
    assert result.exit_code == 0
    assert "Chain result" in result.stdout


@patch("main.Console.input")
@patch("main.AGENT_REGISTRY")
def test_interactive_shell_execution(mock_registry, mock_input):
    mock_agent = MagicMock()
    mock_agent.run.return_value = AgentResult(success=True, content="Shell output")
    mock_registry.get.return_value = lambda: mock_agent
    mock_registry.list_agents.return_value = ["research", "plan"]

    # Test blank line, 'agents', single execution, 'chain', and exit
    mock_input.side_effect = [
        "",
        "agents",
        "research Neural Networks",
        "chain research,plan AI",
        "exit",
    ]
...
@patch("main.Console.input")
def test_interactive_shell_keyboard_interrupt(mock_input):
    mock_input.side_effect = KeyboardInterrupt
    result = runner.invoke(app, ["shell"])
    assert result.exit_code == 0
    assert "Session terminated." in result.stdout
