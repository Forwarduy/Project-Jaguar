...
def _execute_agent(agent_name: str, payload: dict):
    agent_target = AGENT_REGISTRY.get(agent_name)
    if not agent_target:
        console.print(f"El agente '{agent_name}' no está registrado.")
        raise typer.Exit(code=1)

    # Instantiate the agent class/factory returned by the registry
    agent_instance = agent_target() if callable(agent_target) else agent_target
    res = agent_instance.run(payload)

    # Output string content or result object attribute
...
def research_cmd(topic: str = typer.Argument(..., help="Topic to research")):
    ...
    _execute_agent("research", {"topic": topic})


@app.command("plan")
def plan_cmd(goal: str = typer.Argument(..., help="Goal to plan for")):
    """Run the planning agent."""
    _execute_agent("planning", {"goal": goal})
...
def outreach_cmd(
    ...
    _execute_agent("outreach", payload)
