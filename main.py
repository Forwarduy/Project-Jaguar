...
def _run_chain_logic(pipeline_str: str, initial_input: str) -> None:
    ...
    steps = [{"agent": agent_names[0], "arg": initial_input}] + [
        {"agent": name} for name in agent_names[1:]
    ]
...
def research_cmd(topic: str = typer.Argument(..., help="Topic to research")):
    ...
    _print_result(_get_agent_instance("research").run(topic))


@app.command("plan")
def plan_cmd(goal: str = typer.Argument(..., help="Goal to plan for")):
    """Run the planning agent."""
    _print_result(_get_agent_instance("planning").run(goal))
...
def outreach_cmd(
    ...
    res = agent.run(target, message_context=context) if context is not None else agent.run(target)
    _print_result(res)
...
def chain_cmd(
    ...
    _run_chain_logic(pipeline_str, initial_input)


@app.command("shell")
def interactive_shell():
    """Launch interactive REPL mode for multi-agent dynamic execution."""
    console.print(
        Panel.fit(
            "[bold cyan]Project-Jaguar REPL Interactive Shell[/bold cyan]\n"
            "[dim]Commands: agents | health | research <topic> | plan <goal> | "
            "outreach <target> | chain <agents> <input> | exit[/dim]",
    ...
    while True:
        try:
            user_input = console.input("[bold green]jaguar>[/bold green] ").strip()
            if not user_input or user_input.lower() in ("exit", "quit"):
                console.print("Exiting Jaguar REPL shell.")
                break
            ...
            if cmd == "agents":
                list_agents()
            elif cmd in ("research", "plan", "outreach") and arg:
                _print_result(_get_agent_instance(cmd).run(arg))
            elif cmd == "chain" and arg:
                chain_parts = arg.split(maxsplit=1)
                if len(chain_parts) >= 2:
                    try:
                        _run_chain_logic(chain_parts[0], chain_parts[1])
                    except typer.Exit:
                        pass
