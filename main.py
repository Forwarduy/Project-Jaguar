...
# Expose AGENT_REGISTRY instance expected by tests
...
def hello_cmd():
    ...
    typer.echo("Hello from Project Jaguar!")
...
def health_cmd():
    ...
    try:
        ...
        typer.echo("System health check passed.")
...
def outreach_cmd(target: str, context: Optional[str] = None):
    """Execute outreach."""
    typer.echo(f"Outreach to target: {target} with context: {context}")
...
def shell_cmd():
    ...
    shell = ShellSystem()
    typer.echo("Starting Project Jaguar Interactive Shell...")
    while True:
        try:
