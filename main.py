...
def _execute_chain(pipeline_str: str, initial_input: str) -> None:
...
def chain_cmd(
    ...
    _execute_chain(pipeline_str, initial_input)
    ...
    while True:
        try:
            ...
            if cmd == "agents":
                ...
                health_check()
            elif cmd in ("research", "plan", "outreach"):
                if not arg:
                    console.print(f"[red]Error:[/red] '{cmd}' command requires an argument.")
                    continue
                agent = _get_agent_instance(cmd)
                res = agent.run(arg)
                _print_result(res)
            elif cmd == "chain":
                ...
                if len(chain_parts) < 2:
                    console.print(
                        "[red]Error:[/red] Usage: chain <agent1,agent2,...> <initial_input>"
                    )
                    continue
                try:
                    _execute_chain(chain_parts[0], chain_parts[1])
                except typer.Exit:
                    pass
            else:
                console.print(f"[yellow]Unknown command:[/yellow] '{cmd}'. Type 'exit' to quit.")
