...
def _build_client(settings) -> anthropic.Anthropic:
    """Construye el cliente Anthropic oficial usando la config centralizada."""
...
def plan_cmd(goal: str):
    ...
    try:
        ...
        typer.echo("❌ Planning agent not registered.", err=True)
        raise typer.Exit(code=1)

    client = _build_client(settings)
    # PlanningAgent espera project_name + requirements; el CLI hoy solo tiene
    # un --goal. Reutilizamos el mismo texto en los dos hasta que definas
    # si querés separar ambos campos como flags distintos.
    result = agent.execute(project_name=goal, requirements=goal, client=client)
    typer.echo(str(result))
...
def outreach_cmd(target: str, context: Optional[str] = typer.Option(None, "--context", "-c")):
    ...
    try:
        ...
        typer.echo("❌ Outreach agent not registered.", err=True)
        raise typer.Exit(code=1)

    client = _build_client(settings)
    # OutreachAgent exige un "goal". Si no pasaste --context, usamos uno
    # genérico para no volver obligatorio un flag que hoy es opcional.
    goal = context or f"General outreach engagement with {target}"
    result = agent.execute(target_audience=target, goal=goal, client=client)
    typer.echo(str(result))
