...
class ShellSystem(BaseAgent):
    ...
    def __init__(self, registry: Optional[Any] = None) -> None:
        super().__init__(name="shell_system")
    ...
    def execute_command(self, raw_input: str) -> AgentResult:
        ...
        if action == "empty":
            return AgentResult.ok("", metadata={"action": "empty"})
        ...
        if action == "help":
            ...
            return AgentResult.ok(help_text, metadata={"action": "help"})
        ...
        if action == "run":
            if not args:
                return AgentResult.fail("Usage: run <agent_name> [input_data]")
            agent_name = args[0]
        ...
        if action == "chain":
            if not args:
                return AgentResult.fail("Usage: chain <agent1,agent2,...> [initial_input]")
            chain_spec = args[0]
            initial_input = " ".join(args[1:]) if len(args) > 1 else ""
            pipeline = AgentPipeline(registry=self.registry)
            return pipeline.run_chain(chain_spec=chain_spec, initial_input=initial_input)

        # Fallback: handle direct invocation if command matches a registered agent name
        agent_factory = None
